...
import { Mail, ShieldCheck, Calendar, Search, Users, Trash2 } from "lucide-react";
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-50">
              {users.map((user: any) => (
                <tr key={user._id.toString()} className="hover:bg-gray-50/30 transition-all group">
                  ...
                  <td className="px-8 py-5">
                    <RoleSelector userId={user._id.toString()} currentRole={user.role} />
                  </td>
                  ...
                  <td className="px-8 py-5 text-right">
                    <DeleteUserBtn userId={user._id.toString()} userName={user.name} />
                  </td>
          ...
          {users.length === 0 && (
            <div className="p-20 text-center">
              <p className="text-gray-400 font-bold italic underline decoration-rose-500">No users found matching "{query}"</p>
            </div>
          )}
        </div>
      </div>
